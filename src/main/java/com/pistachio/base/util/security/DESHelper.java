...
public class DESHelper {
	...
	public  byte[] decrypt(byte[] d) {
		byte[] key = DESHelper.strDefaultKey.getBytes();
		//下面代码 可不要
		int length = d.length + d.length %8 ;
		if(d.length %8 != 0){
			length = ((d.length / 8) + 1)*8;
		}
		else{
			length = d.length;
		}
	...
	//自定义key解密
	public byte[] encrypt(byte[] s,byte[] key){
		int len = s.length;
		int length = 0;
		if(s.length %8 != 0){
			length = ((s.length / 8) + 1)*8;
		}
		else{
			length = s.length;
		}
	...
	//解密自定义key
	public byte[] decrypt(byte[] d,byte[] key) {
		int length = d.length + d.length %8 ;
		if(d.length %8 != 0){
			length = ((d.length / 8) + 1)*8;
		}
	...
	public  String encryptStr(String str,String keys ){
		byte[] s = str.getBytes();
