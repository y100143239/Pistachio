...
public class CharHelper
{
	...
	private static Logger logger = Logger.getLogger(CharHelper.class);
	...
	public static String ISO2GB(String text)
	{
		...
		{
			logger.error("不支持的编码格式，返回错误对象的字符串", ex);
			result = ex.toString();
		}
		return result;
	}
	...
	public static String GB2ISO(String text)
	{
		...
		{
			logger.error("不支持的编码格式，返回空字符串", ex);
		}
		return result;
	}
	...
	private static String CodeToWord(String text)
	{
		...
		if (Utf8codeCheck(text))
		{
			...
			{
				logger.error("不支持的编码格式，返回null", ex);
				result = null;
			}
		}
		else
